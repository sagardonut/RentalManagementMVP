...
export default function RoomDetails() {
  ...
  if (loading) {
    return (
      <>
        <Navbar />
        <div className="min-h-screen flex items-center justify-center bg-surface dark:bg-slate-900 pt-24">
      ...
      <>
        ...
        <div className="min-h-screen flex items-center justify-center bg-surface dark:bg-slate-900 pt-24">
          <div className="text-center">
            <h1 className="text-4xl font-extrabold text-blue-900 mb-4">Room Not Found</h1>
            <p className="text-slate-500 dark:text-slate-400 mb-8">The room you are looking for doesn't exist.</p>
  ...
  const mapAmenity = (key) => {
    ...
    const labelMap = {
      ...
      'local_parking': 'Parking Included',
    ...
    return {
      icon: iconKey,
      label: labelMap[iconKey] || key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
    };
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className="bg-tertiary-fixed text-on-tertiary-fixed-variant px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1">
                  <span className="material-symbols-outlined text-[14px]">star</span> Top Pick
                </span>
              </div>
              <h1 className="text-4xl md:text-5xl font-bold text-on-surface dark:text-slate-100 mb-2">{room.title}</h1>
              <div className="flex items-center gap-4 text-on-surface dark:text-slate-400">
                <p className="text-2xl font-semibold text-primary">{formattedPrice}<span className="text-sm font-normal text-on-surface dark:text-slate-400">/mo</span></p>
            ...
            {/* Amenities Grid */}
            <div className="bg-surface dark:bg-slate-800 p-8 rounded-xl space-y-6">
              <h2 className="text-lg font-bold tracking-tight uppercase text-on-surface dark:text-slate-400">Amenities & Features</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-6">
                {amenitiesList.map((amenity, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-surface dark:bg-slate-800est flex items-center justify-center flex-shrink-0">
                      <span className="material-symbols-outlined text-primary amenity-icon" style={{ fontSize: '18px' }}>{amenity.icon}</span>
                    </div>
            ...
            <div className="space-y-4">
              ...
              <p className="text-on-surface dark:text-slate-400 leading-relaxed">
                {room.description}
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-28 space-y-6">
              {/* Agency Card */}
              <div className="bg-surface dark:bg-slate-800 p-6 rounded-xl shadow-lg shadow-on-surface/5 border border-outline-variant/20">
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-16 h-16 rounded-full overflow-hidden bg-surface dark:bg-slate-800 flex items-center justify-center">
                    {room.agentId?.avatar ? (
                      <img
                  ...
                  <div>
                    <h4 className="font-bold text-lg">{agencyName}</h4>
                  </div>
                </div>
                ...
                {!user ? (
                  /* Guest View */
                  <div className="relative bg-surface dark:bg-slate-800 rounded-xl p-6 overflow-hidden text-center">
                    <div className="absolute inset-0 backdrop-blur-md bg-surface dark:bg-slate-800/40 z-10"></div>
                    <div className="relative z-20 flex flex-col items-center gap-4">
                      <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center mb-1">
                        <span className="material-symbols-outlined text-primary">lock</span>
                      </div>
                      <h5 className="font-bold text-on-surface dark:text-slate-100 uppercase tracking-widest text-[10px]">Locked Access</h5>
                      <div className="space-y-1 select-none">
                        <p className="text-sm font-mono text-on-surface dark:text-slate-400 blur-[4px]">{agencyPhone}</p>
                        <p className="text-[10px] text-on-surface dark:text-slate-400 blur-[2px]">Kathmandu, Nepal</p>
                      </div>
                      <hr className="w-full border-outline-variant/30 my-1"/>
                      <p className="text-[11px] text-on-surface dark:text-slate-400 leading-tight">
                        Sign up and pay a one-time fee of <span className="font-bold text-on-surface dark:text-slate-100">Rs. 500</span> to unlock all agencies.
                      </p>
                  ...
                  <div className="relative bg-surface dark:bg-slate-800 rounded-xl p-6 overflow-hidden text-center border border-primary/10">
                    <div className="relative z-20 flex flex-col items-center gap-4">
                      <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center mb-1">
                        <span className="material-symbols-outlined text-primary">payments</span>
                      </div>
                      <h5 className="font-bold text-on-surface dark:text-slate-100 uppercase tracking-widest text-[10px]">Unlock Details</h5>
                      <div className="space-y-1 select-none">
                        <p className="text-sm font-mono text-on-surface dark:text-slate-400 blur-[4px]">{agencyPhone}</p>
                        <p className="text-[10px] text-on-surface dark:text-slate-400 blur-[2px]">Kathmandu, Nepal</p>
                      </div>
                      <hr className="w-full border-outline-variant/30 my-1"/>
                      <p className="text-[11px] text-on-surface dark:text-slate-400 leading-tight px-2">
                  ...
                  <div className="space-y-6">
                    <div className="bg-surface dark:bg-slate-800 rounded-xl p-5 border border-primary/20 shadow-sm">
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-on-surface dark:text-slate-100">{agencyPhone}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-on-surface dark:text-slate-100">{agencyName}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <button 
                      ...
                      className="w-full bg-primary text-on-primary font-bold py-4 rounded-xl shadow-xl shadow-primary/20 hover:bg-primary-container hover:-translate-y-0.5 transition-all active:scale-[0.98] block text-center uppercase tracking-widest text-xs"
                    >
                      Book This Room
                    </button>
                  </div>
        ...
        <section className="mt-24 space-y-8">
          <div className="flex justify-between items-end">
            <div>
              ...
              <h2 className="text-3xl font-bold">Similar Sanctuaries nearby</h2>
            </div>
