    ...
    <div className="bg-surface text-on-surface min-h-screen flex" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-surface-container-lowest rounded-2xl p-8 max-w-sm w-full mx-4 shadow-2xl">
            <h3 className="text-xl font-black text-on-surface mb-2">Delete Listing?</h3>
            <p className="text-on-surface-variant text-sm mb-8">This will permanently remove this property from your listings and the database.</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 px-4 py-3 rounded-xl border border-outline-variant text-on-surface font-bold text-sm hover:bg-surface-container-low transition-all">Cancel</button>
      ...
      <aside className="h-screen w-64 fixed left-0 top-0 flex flex-col bg-[#ededf8] py-6 z-40 hidden md:flex shadow-sm">
        <div className="px-6 mb-8">
          ...
          <p className="text-[0.7rem] text-on-surface-variant uppercase tracking-widest font-semibold opacity-70">The Urban Sanctuary</p>
        </div>
          ...
          {navItems.map(item => (
            <button
              ...
              className={`w-full px-4 py-3 flex items-center gap-3 rounded-xl text-left transition-all duration-200 ${activeTab === item.id
                  ? 'bg-white text-primary font-semibold shadow-sm translate-x-1'
        ...
        <div className="border-t border-outline-variant/20 pt-4 px-2 flex flex-col gap-1">
          <a href="#" className="px-4 py-3 flex items-center gap-3 text-on-surface-variant hover:bg-white/50 rounded-xl transition-all">
            <span className="material-symbols-outlined text-xl">help</span>
            <span className="text-[0.875rem]">Help Center</span>
          </a>
          <button onClick={handleLogout} className="w-full px-4 py-3 flex items-center gap-3 text-on-surface-variant hover:bg-white/50 rounded-xl transition-all">
      ...
      <main className="flex-1 md:ml-64 p-8 lg:p-12 pb-24 md:pb-12 min-h-screen">
        ...
        <header className="flex justify-between items-center mb-12">
          <div>
            ...
            <p className="text-on-surface-variant font-medium">
              Welcome back, <span className="font-bold text-on-surface">{user?.fullName?.split(' ')[0] || 'Agent'}</span>. Managing the Sanctuary listings.
            </p>
          </div>
        ...
        {activeTab === 'dashboard' && (
          <>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
              <div className="bg-surface-container-lowest p-8 rounded-xl shadow-[0_8px_24px_rgba(25,27,35,0.04)] flex flex-col justify-between">
                <span className="material-symbols-outlined text-primary mb-4 text-3xl">domain</span>
                <div>
                  <h3 className="text-on-surface-variant text-xs font-bold uppercase tracking-widest mb-1">Total Rooms Listed</h3>
              ...
              <div className="bg-surface-container-lowest p-8 rounded-xl shadow-[0_8px_24px_rgba(25,27,35,0.04)] flex flex-col justify-between">
                ...
                <div>
                  <h3 className="text-on-surface-variant text-xs font-bold uppercase tracking-widest mb-1">Pending Reviews</h3>
                  <p className="text-4xl font-black tracking-tighter text-on-surface">
            ...
            <section>
              <div className="flex items-center justify-between mb-8">
                <h3 className="text-2xl font-black tracking-tight text-on-surface">My Listings</h3>
        ...
        {activeTab === 'rooms' && (
          <section>
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="text-2xl font-black tracking-tight text-on-surface">My Listings</h3>
                <p className="text-sm text-on-surface-variant">{totalRooms} total properties in database</p>
              </div>
              <div className="flex gap-4 items-center">
                <span className="text-sm font-bold text-primary flex items-center gap-1 cursor-pointer">
                  <span className="material-symbols-outlined text-sm">filter_list</span> Filter
                </span>
                <span className="text-sm font-bold text-on-surface-variant flex items-center gap-1 cursor-pointer">
        ...
        {activeTab === 'add' && (
          <section className="max-w-4xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              {/* Basic Info */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-surface-container-lowest p-8 rounded-xl shadow-[0_8px_32px_rgba(25,27,35,0.03)]">
                <div className="space-y-2">
                  <label className="block text-xs font-bold uppercase tracking-widest text-on-surface-variant">Listing Title *</label>
                  <input name="title" value={form.title} onChange={handleFormChange} required
                    className="w-full bg-surface border-0 focus:ring-2 focus:ring-primary rounded-lg p-3 text-on-surface font-medium placeholder:text-outline/50"
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input name="location" value={form.location} onChange={handleFormChange} required
                      className="w-full bg-surface border-0 focus:ring-2 focus:ring-primary rounded-lg p-3 pl-10 text-on-surface font-medium placeholder:text-outline/50"
                      placeholder="Neighborhood, City" type="text" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="block text-xs font-bold uppercase tracking-widest text-on-surface-variant">Monthly Rent (Rs.) *</label>
                  <input name="pricePerMonth" value={form.pricePerMonth} onChange={handleFormChange} required
                    className="w-full bg-surface border-0 focus:ring-2 focus:ring-primary rounded-lg p-3 text-on-surface font-medium placeholder:text-outline/50"
                    placeholder="45000" type="number" min="1" />
                </div>
                <div className="space-y-2">
                  <label className="block text-xs font-bold uppercase tracking-widest text-on-surface-variant">Room Type</label>
                  <select name="type" value={form.type} onChange={handleFormChange}
                    className="w-full bg-surface border-0 focus:ring-2 focus:ring-primary rounded-lg p-3 text-on-surface font-medium">
              ...
              {/* Amenities */}
              <div className="bg-surface-container-lowest p-8 rounded-xl shadow-[0_8px_32px_rgba(25,27,35,0.03)]">
                <h4 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-4">Amenities</h4>
              ...
              {/* Images */}
              <div className="bg-surface-container-lowest p-8 rounded-xl shadow-[0_8px_32px_rgba(25,27,35,0.03)] space-y-4">
                <h4 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant">Property Images</h4>
                <div className="flex gap-3">
                  <input
                    type="url" value={imageUrl} onChange={e => setImageUrl(e.target.value)}
                    className="flex-1 bg-surface border-0 focus:ring-2 focus:ring-primary rounded-lg p-3 text-on-surface font-medium placeholder:text-outline/50"
        ...
        {activeTab === 'profile' && (
          <section className="max-w-lg">
            <h3 className="text-2xl font-black tracking-tight text-on-surface mb-8">My Profile</h3>
            <div className="bg-surface-container-lowest rounded-xl p-8 shadow-sm space-y-4">
              <div className="flex items-center gap-5 mb-6">
                ...
                <div>
                  <p className="font-black text-on-surface text-xl tracking-tight">{user?.fullName}</p>
              ...
              <div className="border-t border-slate-100 pt-4 space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  <span className="text-on-surface-variant font-medium">Agency</span>
                  <span className="font-semibold text-on-surface">The Urban Sanctuary</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-on-surface-variant font-medium">Total Rooms</span>
      ...
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 w-full bg-surface-container-lowest border-t border-outline-variant/20 flex justify-around py-3 z-50">
        {navItems.map(item => (
          <button key={item.id}
            onClick={() => item.id === 'add' ? openAddForm() : setActiveTab(item.id)}
            className={`flex flex-col items-center gap-0.5 transition-colors ${activeTab === item.id ? 'text-primary' : 'text-on-surface-variant'}`}>
  ...
  if (rooms.length === 0) {
    return (
      <div className="text-center py-20 bg-surface-container-low rounded-2xl border-2 border-dashed border-outline-variant">
        ...
        <h3 className="text-lg font-bold text-on-surface mb-1">No listings yet</h3>
        <p className="text-sm text-on-surface-variant">Add your first property to get started.</p>
      </div>
    );
  }
  return (
    <div className="bg-surface-container-lowest rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(25,27,35,0.03)]">
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="bg-surface-container-low border-b border-outline-variant/10">
            {['Property', 'Location', 'Monthly Rent', 'Status', 'Actions'].map((h, i) => (
              <th key={h} className={`px-6 py-4 text-xs font-bold text-on-surface-variant uppercase tracking-wider ${i === 4 ? 'text-right' : ''}`}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody className="divide-y divide-outline-variant/10">
          {rooms.map(room => (
            <tr key={room._id} className="group hover:bg-surface-container-low/50 transition-colors">
              <td className="px-6 py-4">
                <div className="flex items-center gap-4">
                  {room.images?.[0] ? (
                    <img src={room.images[0]} alt={room.title} className="w-16 h-12 object-cover rounded-lg flex-shrink-0" />
                  ) : (
                    <div className="w-16 h-12 bg-surface-container-high rounded-lg flex-shrink-0 flex items-center justify-center">
                      <span className="material-symbols-outlined text-outline text-2xl">home</span>
                    </div>
                  )}
                  <div>
                    <div className="font-bold text-on-surface">{room.title}</div>
                    <div className="text-xs text-on-surface-variant">{room.type} • {room.amenities?.includes('Furnished') ? 'Furnished' : 'Unfurnished'}</div>
                  </div>
                </div>
              </td>
              <td className="px-6 py-4 text-sm text-on-surface-variant font-medium">{room.location}</td>
              ...
              <td className="px-6 py-4 text-right">
                <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onEdit(room)} className="text-on-surface-variant hover:text-primary transition-colors p-1" title="Edit">
                    <span className="material-symbols-outlined text-xl">edit</span>
                  </button>
                  <button onClick={() => onDelete(room._id)} className="text-on-surface-variant hover:text-error transition-colors p-1" title="Delete">
