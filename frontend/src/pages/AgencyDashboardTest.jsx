    ...
    <div className="bg-surface text-on-surface flex min-h-screen">
      ...
      <aside className="h-screen w-64 fixed left-0 top-0 flex flex-col bg-surface-container py-6 gap-2 z-40 hidden md:flex shadow-sm">
        ...
        <nav className="flex flex-col gap-1 px-2 flex-1">
          <button className="w-full px-4 py-3 flex items-center gap-3 rounded-xl text-left bg-surface-container-lowest text-primary font-semibold">
      ...
      <main className="flex-1 md:ml-64 p-6 md:p-10">
        ...
        <div className="text-center py-20">
          ...
          <h3 className="text-2xl font-bold text-on-surface mb-2">Agency Dashboard</h3>
          <p className="text-on-surface-variant">This is a test version to verify the dashboard loads correctly.</p>
        </div>
      </main>
    </div>
  );
