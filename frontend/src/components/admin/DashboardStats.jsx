...
const StatCard = ({ label, value, trend, isRevenue, loading }) => {
  if (loading) {
    return (
      <div className={`p-8 rounded-[32px] bg-white border border-slate-50 animate-pulse ${isRevenue ? 'col-span-2' : ''}`}>
      ...
      <div className="p-10 rounded-[40px] bg-[#0040A1] text-white flex flex-col justify-between relative overflow-hidden group">
        ...
        <div className="mt-8 flex items-end gap-[6px] h-12">
          {[40, 70, 45, 90, 60, 100].map((h, i) => (
            <div 
              key={i} 
              className="flex-1 rounded-sm bg-white/20 group-hover:bg-white/30 transition-all duration-500" 
              style={{ height: `${h}%` }}
            ></div>
          ))}
        </div>
  ...
  return (
    <div className="p-8 rounded-[36px] bg-white border border-slate-50 hover:shadow-2xl hover:shadow-blue-100/40 transition-all duration-500 group">
      ...
      <h3 className="text-4xl font-black text-slate-800 tracking-tighter group-hover:scale-105 transition-transform origin-left duration-500">{value}</h3>
    </div>
  );
};
