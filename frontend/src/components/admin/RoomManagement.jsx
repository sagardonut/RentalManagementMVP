    ...
    <div className="bg-surface dark:bg-slate-800 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
      ...
      <div className="p-6 border-b border-surface-container">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h2 className="text-xl font-bold text-on-surface dark:text-slate-100">All Rooms</h2>
      ...
      {loading ? (
        <div className="p-12 text-center">
          ...
          <p className="text-on-surface dark:text-slate-400 text-sm font-bold uppercase tracking-widest">Loading rooms...</p>
        </div>
      ) : (
        <>
          {/* Rooms Grid */}
          <div className="p-6">
            {filteredRooms.length === 0 ? (
              <div className="text-center py-12">
                <span className="material-symbols-outlined text-6xl text-on-surface dark:text-slate-400 mb-4 block">bed</span>
                <h3 className="text-xl font-bold text-on-surface dark:text-slate-400 mb-2">No Rooms Found</h3>
                <p className="text-on-surface dark:text-slate-400 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredRooms.map((room) => (
                  <div key={room._id} className="bg-surface dark:bg-slate-900 rounded-xl overflow-hidden border border-outline-variant/20 hover:shadow-lg transition-shadow">
                    {/* Room Image */}
                    ...
                    <div className="p-5">
                      <div className="flex justify-between items-start mb-3">
                        <h3 className="font-bold text-lg text-on-surface dark:text-slate-100 line-clamp-1">{room.title}</h3>
                        <button
                      ...
                      <div className="space-y-2 mb-4">
                        <div className="flex items-center text-sm text-on-surface dark:text-slate-400">
                      ...
                      <div className="flex gap-2">
                        <button
                          onClick={() => onEdit(room)}
                          className="flex-1 px-4 py-2 bg-surface dark:bg-slate-800 border border-outline-variant/20 text-on-surface dark:text-slate-100 rounded-lg font-bold text-sm hover:bg-surface dark:bg-slate-800 transition-colors"
                        >
