...
export default function AgentCard({ agent, onViewListings }) {
  return (
    <div className="bg-surface-container-lowest rounded-xl p-6 shadow-sm border border-outline-variant/10 hover:shadow-md transition-all duration-300 group">
      <div className="flex items-start gap-5 mb-6">
        ...
        <div className="flex-1">
          ...
          <h4 className="text-xl font-extrabold text-on-surface group-hover:text-primary transition-colors">{agent.fullName}</h4>
          <p className="text-xs text-on-surface-variant font-medium">{agent.specialization || "Residential Specialist"}</p>
        </div>
      </div>
